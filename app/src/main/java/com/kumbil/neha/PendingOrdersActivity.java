...
public class PendingOrdersActivity extends AppCompatActivity implements CreateAlert.OnCompleteListener{
        ...
        getOrdersCall.enqueue(new Callback<ordersResp>() {
            ...
            public void onResponse(Call<ordersResp> call, Response<ordersResp> response) {
                ...
                if(resp.getStatus() == 0 && resp.getOrdrs().length > 0 )
                {
                    ...
                    RecyclerView rvOrders = (RecyclerView) findViewById(R.id.pending_orders_rv);
                    ...
                    Toast.makeText(PendingOrdersActivity.this,resp.getMessage(),Toast.LENGTH_SHORT).show();
                }
            }
        ...
        if(ok) {
            ...
            getOrdersCall.enqueue(new Callback<ordersResp>() {
                ...
                public void onResponse(Call<ordersResp> call, Response<ordersResp> response) {
                    ordersResp resp = response.body();
                    if(resp.getStatus() == 0 && resp.getOrdrs().length > 0 ) {
                        Intent intn = new Intent(getApplicationContext(),PendingOrdersActivity.class);
                        startActivity(intn);
                        finish();
                    }
                }
